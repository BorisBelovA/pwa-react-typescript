...
const CardProfile = (props: Props) => {
  ...
  const header = (
    <>
      <Typography variant='h1'>{`${person.firstName}, ${person.birthday ? calculateAge(person.birthday) : 0}`}</Typography>
      <Typography variant='body1' className={styles.head}>{!!info.who ? whoOptions[info.who] : 'By self'}</Typography>
    </>
  )
